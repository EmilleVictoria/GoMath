...
var qNum = 0;
var corretas = []; //array to show the correct corretas and false ones
var minutes = 0;
var seconds = 0;
...
function appendQuestion() {
  ...
  var divBody = `<h3 class='quizHeader'>Q: ${questions[qNum].enunciado}</h3>`;
  divBody += "<ul class='option_group' id='option_group'>";
  for (var i = 2; i < questions[qNum].length; i++)
    escolhida = verificarAlt(i);
    divBody += `<li class='option' onclick='activeOpt(this)'>${questions[qNum].escolhida}</li>`;
...
function activeOpt(id) {
  ...
  id.className = "active";
  if (id.innerHTML === questions[qNum].correta) corretas[qNum] = true;
  else corretas[qNum] = false;
}
function nxtQuestion() {
  if (!(typeof corretas[qNum] === "undefined")) {
  ...
  } else alert("selecione alguma alternativa!");
}
function appendResult() {
  var correctQuestions = 0; // number of questions that were respondidas correctly
...
function homePageReAttempt() {
  ...
  corretas = []; //clear the correct corretas
...
function appendAllQuestions() {
  for (var j = 0; j < questions.length; j++) {
    ...
    var corretaVal = questions[j].correta;
    ...
    for (var i = 0; i < numOfOptions; i++) {
      if (optionVals[i] === corretaVal)
        divBody += `<li class='optionPanel active'>${optionVals[i]}</li>`;
      //active class assigned to this option if it matches the correta
      else divBody += `<li class='optionPanel'>${optionVals[i]}</li>`; //options which does'nt match the correta
    }
  ...
  if (!addingRec) {
    ...
    for (var i = 0; i < 4; i++)
      htmlDesign += `<li style="display: flex;" class="panelLi"><h3>Option ${
        i + 1
      }:&nbsp</h3><input class="form-control w-50"></li>`;
    htmlDesign +=
      '<li style="display: flex;"><h3>correta:&nbsp</h3><input class="form-control w-50"></li>';
...
function addRec(id) {
  ...
  var corretaVal = id.parentNode.previousSibling.childNodes[1].value;
  ...
  if (!editingRec) var key = firebase.database().ref("Quetoes").push().key;
  //generate a key for the Questions object
  // if editRec called this function, then we have to get the question key from the button
  else var key = id.id;
  // check if any field is empty
  if (!(questionVal === "") && !(corretaVal === "")) {
    //if the fields are filled, then process further
    ...
    if (enteredOptions !== 2) alert("Please enter atleast two options");
      ...
      // check of the correta matches any of the options
      enteredOptions = 0;
      for (var i = 0; i < optionVals.length; i++) {
        if (optionVals[i] === corretaVal) {
          enteredOptions = 1; //this flag indicates that the correta matches one of the corretas
          break;
        }
      }
      if (!enteredOptions)
        alert("Please enter one of the options in correta filed");
      else {
        ...
        if (editingRec) {
          ...
          currentEditQuestion.correta = corretaVal;
          for (var i = 0; i < numOfOptions; i++)
          ...
          for (var i = 0; i < currentEditQuestion.options.length; i++) {
            if (currentEditQuestion.options[i] === currentEditQuestion.correta)
              divBody += `<li class='optionPanel active'>${currentEditQuestion.options[i]}</li>`;
            //active class assigned to this option if it matches the correta
            else
              divBody += `<li class='optionPanel'>${currentEditQuestion.options[i]}</li>`; //options which does'nt match the correta
          }
        } else {
          ...
          currentQuestion.enunciado = questionVal; //insert question in the object
          currentQuestion.correta = corretaVal; //insert correta value in the object
          for (var i = 0; i < numOfOptions; i++)
          ...
          for (var i = 0; i < currentQuestion.options.length; i++) {
            if (currentQuestion.options[i] === currentQuestion.correta)
              divBody += `<li class='optionPanel active'>${currentQuestion.options[i]}</li>`;
            //active class assigned to this option if it matches the correta
            else
              divBody += `<li class='optionPanel'>${currentQuestion.options[i]}</li>`; //options which does'nt match the correta
          }
        }
        //now lets add edit and remove buttons
        divBody += "<li style='display:flex; justify-content: center;'>";
        ...
        var data = {
          key: key,
          enunciado: questionVal,
          options: optionVals,
          correta: corretaVal
        };
        firebase.database().ref(`Quetoes/${key}`).set(data); //set means that we dont want to overwrite the data but save in the same key as we did above, we used push above now we have to use set here
      }
    }
  ...
  if (!addingRec) {
    ...
    for (var i = 0; i < numOfOtions; i++) {
      ...
      if (
        ...
        corretaVal = optionVals[i]; //assign this option value to the correta object
    }
    //we will store the whole li which has our quesion,correta and options to the 'rVal', so if we do'nt update the edited question, we may restore the recent value
    ...
    htmlDesign += `<li style="display: flex;"><h3>correta:&nbsp</h3><input class="form-control w-50" value="${corretaVal}"></li>`;
    htmlDesign += '<li style="display: flex; justify-content: center;">';
